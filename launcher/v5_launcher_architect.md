...
## 改修ログ（2026-07-02 その4：選手ラベルのデータ取得で試合スタッツを確実に拾う）

`fetch_label_data`（Step2 ラベルチップの「📊」ボタン）で選手データを取得する際、`matchStats`（今試合のFotMob rating/Goals/Assists等）は `facts.matchData` が既に取得済みの場合のみ計算されていた。ユーザーが試合ラベルより先に選手ラベルをクリックすると matchStats が空のままになっていた。

**対処**（dashboard.js `fetch_label_data` の player 分岐）: `matchData` 未取得なら、同案件のラベル一覧（`factsForClient.labels`）から match ラベルを探し、選手データ取得前に `fetchMatch` を先読み実行。取得順に関わらず、試合の話題なら選手データに今試合のスタッツが乗る。playerId での紐付け（`matchData.playerStats[playerId]`）は実データ（日本vsブラジル・佐野海舟）で動作確認済み。

※「📊 データ全取得」（`fetch_data` 一括版）は元々ラベル配列を一括処理するため、match→player の順で同一リクエスト内処理されており対象外（既に正常）。

---

## 改修ログ（2026-07-02 その5：「mods 不足」再発 — 救済汎用化 + facts永続化）

その3の修正（プロンプト表記変更＋固定キー救済）後も同エラーが再発。原因は2つ複合していた:

1. **AIの出力キー名は非決定的**: 固定リスト `['slides','slide','contents']` 以外のキー名で返された場合は救済できず「mods 不足: 0枚」に戻ってしまう
2. **ダッシュボード再起動でfacts劣化**: `session.facts`/`factsCache` はメモリ上のみで保持されており、再起動後は `generate_script` 等が `factsForClient`（Step2表示用の軽量版。**記事本文なし・matchDataは scoreline のみ**）にフォールバックしていた。素材が薄くなるほどAIの出力が不安定になりやすい

**対処**:
- `script_gen.js`: `extractModsArray()` を新設。キー名を問わず「narration/titleを持つオブジェクトが4つ以上並ぶ配列」を救済対象として探す汎用ロジックに変更。それでも見つからない場合は「トップレベルキーは必ずmods」と矯正指示を追記して**1回だけ自動リトライ**してから諦める
- `dashboard.js`: `runResearch` で完全版 `facts` 自体も `topicData[topic].facts` として保存（`saveTopicData`/`loadTopicData` で永続化）。`generate_brief`/`generate_script`/`runRender` の3箇所のフォールバックで `factsForClient` より先に `facts` を優先するよう変更 → 再起動後も記事本文・matchData全体を保って脚本生成できる

**注意**: 2026-07-02以前に保存された `output/topic_data.json` の既存トピックには `facts` フィールドが無いため、旧トピックは一度「情報収集」をやり直すまで軽量版フォールバックのまま。新規に収集したトピックからは自動的に完全版が保存される。

---

## 改修ログ（2026-07-02 その6：「画面真っ暗」報告の検証 → 2つの実バグを特定・修正）

ユーザー報告「レンダリングした動画が画面真っ暗」を実動画（フレーム抽出・ffmpeg blackdetect）で検証し、2つの独立したバグを確認。

### バグA: history/statsスライドの背景欠落（既知・その2で対処済みの再確認）

その2で修正した「history/insightにsiBindingが無いと背景画像が付かない」問題そのもの。history.js:140 は `bgImage` が無いと `PALETTE.surface`（単色濃紺）にフォールバックするため、画面の大部分が単色パネルになり「真っ暗」に見えていた。**該当動画はその2の修正コミット以前にレンダリングされたものだった**（dashboard.js等のコード修正はプロセス再起動しないと反映されないため、動いていたダッシュボードには旧コードのまま）。再検証レンダリングで解消を確認（ギャラリープリセット1枠→2枠に増加、stats/historyとも背景写真あり）。

### バグB（新規発見）: 動画末尾の音声欠落

検証レンダリング中に ffprobe で発見: 映像103.08s に対し音声103.07s→実際は94.9sしかない旧final.mp4が生成されていた（約8.2秒＝endingスライド1枚分がまるごと無音）。

**原因**: [concat.js](launcher/concat.js) の `concatVideos()` が `-f concat`（ストリームコピー相当）で、各スライド個別にAACエンコードされた `muxed_*.mp4` を単純連結していた。個々のファイルは単体デコードでは無害だが、連結後にセグメント境界でAACのチャンネル設定(PCE)不整合が発生し、後段のBGMミックス（要デコード）で `Error submitting packet to decoder` → BGMミックス失敗 → 無音フォールバック的挙動になっていた。

**対処**: `concatVideos()` を `-f concat` デマルチプレクサから **concat フィルタ**（`-filter_complex "[0:v][0:a][1:v][1:a]...concat=n=N:v=1:a=1"`、全セグメントを一度PCM/生フレームにデコードしてから結合・再エンコード）に変更。セグメント境界のビットストリーム不整合を構造的に回避。既存の生成済み音声・動画ファイルで再結合をテストし、音声/映像がほぼ完全一致（103.07s/103.08s）することを確認（追加API課金なしで検証可能だった）。

※ 反映には `node dashboard.js` の再起動が必要。

---

## 改修ログ（2026-07-02 その7：コメントカードの見切れ修正）

前項の検証スクショで発見。コメント吹き出しが斜めに傾いた状態で左端（Xアイコン）が画面外に切れていた。原因は2つ複合:

1. **テキスト長とボックス高さの不整合**: `slides/comments.js` の `_lineCount()` は68字超を一律「3行」判定するが、呼び出し元 `script_gen.js injectRealComments` のテキスト切り詰め上限は120字。3行分(34字×3=102字)を超える最大18字分がカードの固定高さからはみ出していた
2. **回転軸が中央固定**: `.cmt-card` の `rotate()` がデフォルトで中心を軸にしていたため、左寄せカード(`align-self:flex-start`)を回転させると軸が中央にある分だけ左端が親要素(`.cmt-overlay`, `overflow:hidden`)の外にはみ出して切られていた

**対処**:
1. `_packComments()` に `_truncateForLines()` を追加。各コメントを割り当て行数(行数×34字)に収まる長さへ「…」付きで切り詰めてから使用
2. `--rot-origin` カスタムプロパティを追加し `transform-origin` に反映。左寄せカードは `left center`、右寄せカードは `right center` を軸に回転させ、固定されている側の端は動かさず逆側（余白のある側）にだけ傾くようにした。傾き演出自体は維持

120字ちょうどの長文コメントでPuppeteerスクリーンショット検証（追加APIコストなし）。3行に収まった省略テキスト表示・アイコン完全表示を確認。

※ 反映には `node dashboard.js` の再起動が必要。

---

## 改修ログ（2026-07-02 その8：コメントカード見切れ、実は直っていなかった → 真因はactive強調のscale）

その7の修正（テキスト切り詰め＋回転軸変更）をコミット後、ユーザーから「まだ直ってない、1番上のコメントが右端側に見切れる」と再報告。**単発のCSSオーバーレイだけの検証では見えていなかった不具合**で、実際の本番HTML（`buildStatsHTML` + `injectCommentOverlay`）をPuppeteerで**読み上げ中のactive強調演出の瞬間**まで進めてスクリーンショットを撮ることで再現・特定した。

**真因**: コメントが読み上げられている間、`.cmt-card` には `scale(1.04)` と `rotate()`（最大±2deg）が同時にかかる（[slides/comments.js](launcher/slides/comments.js) の `cmtActive_${i}` keyframes）。1番上のスロットは `.cmt-overlay` の天井(`top`)との余白がゼロに近く、scale+rotateの複合でキャンバス上端(y=0)そのものを超えて描画されていた。**CDPスクリーンショットは1920x1080の範囲外を原理的に描画できない**ため、`overflow:hidden` をどこで外しても直らない種類の不具合だった（その7の transform-origin 修正自体は「傾き単体」では有効だが、active強調のscaleとの複合までは考慮できていなかった）。

**対処**:
1. active強調時の `scale(1.04)` → `scale(1.02)` に縮小（拡大の演出自体は維持）
2. `.cmt-overlay` の `top` を `130px` → `175px` に拡大し、1番上のスロットにscale+rotate分の逃げ代を確保

実際のstats slide HTMLで、1番目(左寄せ, rot=-2deg最大値)・2番目(右寄せ)双方のactive強調タイミングをPuppeteerで`document.getAnimations()`のcurrentTimeを直接指定して再現し、スクリーンショットで両方とも枠内に収まることを確認（追加APIコストなし。実際の動画レンダリングと同じCDPスクリーンショット方式で検証）。

**教訓**: コメントオーバーレイのような時間経過で見た目が変わる要素は、静止状態のCSSだけでなく**アニメーションの全状態（特にactiveな強調演出中）を実際の描画パイプラインで検証する**こと。

※ 反映には `node dashboard.js` の再起動が必要。

---

## 改修ログ（2026-07-02 その9：字幕ズレ修正 / 選手データ取得フォールバック）

### 字幕バーの音声ズレ（slides/subtitles.js）

字幕表示タイミングが音声からナレーション後半になるほどズレる不具合。実際のナレーション音声をWhisper APIに通して確認したところ、**同音異義語("得点"→"特典")や固有名詞("エムバペ"→"m","バ","ペ"に分解)の誤認識**が頻発していた。従来は「原文の文字数累積比率」と「Whisper認識結果の文字数累積比率」が一致する前提で字幕タイミングをマッピングしていたが、誤認識区間で両者の文字数が食い違うと、その差が後続すべての字幕タイミングに蓄積していた。

`_alignTextToWhisperTime()` を新設: 原文とWhisper認識結果を**編集距離(レーベンシュタイン距離)アラインメント**し、一致/置換した文字ペアからタイムスタンプを取得。誤認識区間は不一致として読み飛ばし、前後の確定点から線形補間する。実データで検証: 旧方式は実際の発話開始(5.74s)より0.44秒早いタイミングを算出していたが、新方式は5.74sに一致。

### 選手データ取得のフォールバック（research.js / dashboard.js）

FotMobのグローバル選手検索(`fetchPlayer`)が表記ゆれ・マイナー選手・同姓等でヒットしない場合、選手データが一切取れない問題があった（例:「ゴードン」）。

`findPlayerInMatchData(matchData, name)` を新設。`fetchMatch()` の結果には試合に出場した**全選手**の攻守スタッツ(`matchData.playerStats`、id別)が既に含まれているため、グローバル検索が失敗しても既取得の試合データ内から名前一致(完全一致→部分一致→姓のみ一致、アクセント無視)で探せば追加APIコールなしで拾える。`fetch_label_data`（単一ラベル）・`fetch_data`（一括）の両方に組み込み、取得元が試合データ由来の場合はUIに「（試合データより）」と表示。`compressFacts`（script_gen.js）も`playerData.matchStats`をAIプロンプトに渡すよう拡張（従来は season stats のみ）。

※ 反映には `node dashboard.js` の再起動が必要。

---

## 改修ログ（2026-07-02 その10：今季スタッツがAIに一切渡っていなかった重大バグを修正）

ユーザーから「トナーリ案件、今季スタッツ取れてる?」と確認があり調査したところ、**`compressFacts()`（script_gen.js）が存在しないフィールド名を参照しており、今季スタッツが常にAIへ渡らず`undefined`になっていた**ことが判明。

**詳細**: `fetchPlayer()`（fetchers/fotmob_player.js）の実際の戻り値は今季スタッツを `pd.seasonStats.{goals,assists,rating,tackles,interceptions,...}` にネストして返すが、`compressFacts()` は存在しない `pd.goals` / `pd.assists` / `pd.rating` / `pd.stats`（フラットなキー）を参照していた。FotMob自体は正常にデータを取得できていた（例: トナーリ G0 A2, タックル35, インターセプト31, パス成功率84.4%）が、AIプロンプトに渡す段階で毎回消えていた。

**影響範囲**: Step2のUI表示（`dashboard.js`/`web/index.html`）は`playerData.seasonStats`を正しく参照していたため**画面上は今季スタッツが正しく表示されており、この不具合はUIからは気づけなかった**。実際に脚本生成AIへ渡るプロンプト内容を直接確認するまで発覚しなかった、サイレントな品質劣化バグ。statsスライドの脚本が実質データなしで（AIの一般知識頼みで）生成されていた可能性がある。

**対処**: `pd.seasonStats` から正しく `goals`/`assists`/`rating`/`appearances`/`leagueName`/詳細stats一式を抽出するよう修正。実データ(Tonali)でAIプロンプトに正しく反映されることを確認。

**教訓**: 「データが取れているか」はUI表示だけでなく、**実際にAIへ渡るプロンプトの中身を直接確認する**こと。中間の変換関数(compressFacts等)にフィールド名の取り違えがあっても、UIとAIが別々の場所から参照していると片方だけ壊れていても気づきにくい。
